...
from .Blocks.FPN import FPNNeck_ResNet
from .encoders import get_encoder


class SF_ResNet(nn.Module):
    def __init__(self, encoder_name='resnet50', neck_name='fpn', out_channels=1, inplanes=128, pretrained='imagenet'):
        super(SF_ResNet, self).__init__()
        self.encoder = get_encoder(encoder_name, in_channels=3, depth=5, weights=pretrained)
        self.neck = FPNNeck_ResNet(inplanes=inplanes, neck_name=neck_name)
        self.deconv_for_change_5 = DeConv(16 * inplanes, inplanes, scale=8)  # change5--> 64*64*inplanes
        self.deconv_for_change_4 = DeConv(8 * inplanes, inplanes, scale=4)   # change4--> 64*64*inplanes
        self.deconv_for_change_3 = DeConv(4 * inplanes, inplanes, scale=2)   # change3--> 64*64*inplanes
        self.conv_for_change_2 = Conv3Relu(2 * inplanes, inplanes)  # change2--> 64*64*inplanes
